...
- Drop the push trigger the release run already covers
  ([`589434e`](https://github.com/datapointchris/syncer/commit/589434ee5a44ae6e9ae5a9d73c965b3b5fc57acb))
...
- Strip local deployment details from a public repo
  ([`0a5dd14`](https://github.com/datapointchris/syncer/commit/0a5dd1428c3c9cf4da43884e56f5791e13d9788e))

syncer is public, and it carried the specifics of one machine setup as load-bearing prose: a private
  registry path in CLAUDE.md and two test fixtures, "the work box", "Syncthing" and "a shared
  dotfiles bucket" as the justification for the scaffolding rules, a machine roster in the v5.0.0
  migration marker, and two sibling tool names in the README and in the help text `config init`
  prints.

None of it was resolvable by someone installing the tool, and the design reasoning never depended on
  it. Every incident that justifies a decision is kept and restated in terms any reader shares — a
  config deployed from a shared source pointing one machine at a directory that only existed on
  another teaches the same lesson without naming a locale.

CLAUDE.md already called the registry-sharing "a fleet fact, not a syncer fact" twelve lines above
  one of the violations, so the rule now lives in the standards with a sweep rather than being
  restated per repo.

### Features

- **policy**: Make the rule surface browsable and patchable
  ([`ea1a465`](https://github.com/datapointchris/syncer/commit/ea1a46543924cdd6b244245d173b703de1768fbe))

Finding the rule to change meant knowing the "<selector>:<state>" grammar and cross-producing two
  vocabularies in your head, and changing one cell meant inventing a named policy that restated the
  nine rules it was based on. Both are gone.

`policy rules [policy]` prints every settable key grouped by state, with that state's meaning, the
  actions it accepts, what the policy decides now, and which selector decided it — one flat table
  you can grep. The rule space is enumerable (three role selectors x the state enum), so it never
  needed assembling by hand. `policy actions list` is the vocabulary and `policy actions show
  <action>` the drill-down: what it runs, every precondition that refuses it, and what it will never
  do.

A [policies.X] table now merges onto whatever X already is, so patching a built-in and defining a
  policy are one syntax:

[policies.standard.rules] "*:gone" = "delete_local"

Merging happens on the raw dict. Building a Policy from the table first would fill absent fields
  with model defaults and clobber the base — a patch to [policies.mirror.rules] would have reset
  mirror's scope from all to tracked. `extend` names a base only when the name is not a built-in's,
  and unknown keys are rejected rather than ignored, since pydantic would drop `extends` in silence
  and a policy that never inherited reads as the whole block being ignored.

ACTION_DOCS lives beside the guards it describes. `applies_to` is declared rather than derived
  because only _delete_local tests state.primary — the rest check live counts, and adding a primary
  check to make it derivable would have a guard trusting classify-time state, which invariant 6
  exists to prevent. It is proven instead: every mutator is driven against a repo genuinely in each
  state, asserting applies_to is exactly where it can act. Both directions, since the outside
  direction alone would let a doc widen to "any state" and still pass.

Refusals are now keyed. Refusal is a StrEnum, REFUSAL_TEXT holds the wording once, and
  Outcome.reason carries the key beside the display-only message. A test joining two tables on an
  English sentence fails whenever someone rewrites it with nothing wrong, which is how such a test
  gets loosened to a substring and then passes for reasons nobody chose — the pre-existing
  assertions had already degraded to `'dirty' in message`.
